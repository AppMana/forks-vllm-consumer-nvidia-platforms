...
class DeepseekV4SWACache(torch.nn.Module, AttentionLayerBase):
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec:
        ...
        return SlidingWindowMLASpec(
            ...
            # fp8_ds_mla: the decode kernel's packed page; int8_ds_mla: 512B
            # signed-int8 row + fp32 row scale + 12B pad = 528B per token.
            state_content_bytes=(
                self.packed_bytes_per_token
                if uses_fp8_ds_mla_layout
                else (528 if uses_int8_ds_mla_layout else None)
            ),
            # FlashMLA packing stride; 528B for the int8_ds_mla packed layout;
            # 512B for FlashInfer sparse (#44577) otherwise.
            alignment=(
                self.packed_page_alignment
                if uses_fp8_ds_mla_layout
