...
def _get_packed_kv_cache_layout(
    ...
    Every slot offset and the block stride are rounded up to 16 bytes. Some
    byte-packed KV formats store wider values inside the uint8 backing tensor
    (int8_ds_mla keeps an fp32 row scale), and several cache writers use 16-byte
    vectorized stores (the csrc fp8_ds_mla uint4 path). Without the alignment,
    consecutive blocks alternate 16B alignment and those writers fault with
    "CUDA error: misaligned address" -- observed with block stride 42696, which
    is 8 mod 16, on the int8_ds_mla PP=10 deploy.
    """
    ...
    for group in kv_cache_groups:
        ...
        block_stride = max(block_stride, round_up(byte_offset, 16))
