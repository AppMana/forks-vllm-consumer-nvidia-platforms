...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def warmup_pp_decode_update(self) -> None:
        """JIT-compile the kernels behind ``update_pp_decode_requests``.

        That path only runs on real steps, and the sampled-token broadcast is
        disabled for the whole warmup window, so the warmup steps never reach
        it on non-last PP ranks. Its first triton compile must not happen
        ...
        deadlocks the pipeline. The outputs come from ``PPHandler`` sliced
        exactly like a received payload, so ``post_update`` specializes on the
        same strides and alignment as at serving time. An all -1 idx_mapping
        makes the update a no-op; the draft scatter is a plain index_copy_
        with nothing to compile and is skipped.
        """
        assert self.pp_handler is not None
        outputs = self.pp_handler.warmup_sampled_outputs()
        outputs["proposed_tokens"] = None
        self.postprocess_sampled(**outputs)
    ...
    def prepare_inputs(
        ...
        if scheduler_output.replayed_pp_anchor_req_ids:
            ...
            replayed_pp_anchor = async_tensor_h2d(
        ...
        if not draft_tokens and self.model_state.num_new_sampled_tokens_per_step == 1:
            ...
            if self.pp_handler is not None:
                ...
                num_logits = np.minimum(
                    num_scheduled_tokens_np, num_draft_tokens_per_req + 1
                ).astype(np.int32)
                num_draft_per_req_gpu = async_tensor_h2d(
    ...
    def sample(
        ...
        if input_batch.num_reqs == 0:
            ...
            with record_function_or_nullcontext("gpu_model_runner: spec_verify"):
    ...
    def execute_model(
        ...
        # Run model.
        with record_function_or_nullcontext("gpu_model_runner: target_forward"):
