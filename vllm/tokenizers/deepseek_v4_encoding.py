...
# adds nothing.
#
# NOTE (DeepSeek-V4-Flash-0731): this replaced a single REASONING_EFFORT_MAX
# constant that only fired for `max`, with `high` accepted but emitting nothing.
# The strings below are verbatim from the 0731 release's
# encoding/encoding_dsv4.py -- `high` now emits what `max` used to, and `max`
# emits new, stronger text. Do not paraphrase: the prefix is the entire
# mechanism, so any drift silently changes model behaviour.
