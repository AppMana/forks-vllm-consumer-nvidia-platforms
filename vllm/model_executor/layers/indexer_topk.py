...
class SparseIndexerTopk(torch.nn.Module):
    ...
    def _resolve_auto(
        ...
        persistent_topk.
        """
        if _should_use_sm120_short_row_topk_decode(
            topk_tokens,
            logits.shape[1],
            num_rows,
            self._is_cuda and current_platform.is_device_capability_family(120),
        ):
            # GB10's DSV4 indexer keeps its decode on the row-local selector
            # written for consumer Blackwell instead of re-entering
            # persistent_topk.
            return "per_row"
