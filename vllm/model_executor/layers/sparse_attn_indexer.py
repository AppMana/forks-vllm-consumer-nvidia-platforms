...
def _exact_slab_keys_kernel(
    ...
    val = tl.load(cand_val_ptr + row * K + offs_k, mask=mask_k, other=float("-inf"))
    valid = idx >= 0
...
def warmup_indexer_prefill_topk_kernel(device: torch.device) -> None:
    ...
    logits = torch.zeros((num_rows, context_rows), dtype=torch.float32, device=device)
    row_starts = torch.zeros(num_rows, dtype=torch.int32, device=device)
    row_ends = torch.full((num_rows,), context_rows, dtype=torch.int32, device=device)
...
def warmup_indexer_streaming_topk_kernels(device: torch.device) -> None:
    ...
    slab_logits = torch.zeros((num_rows, slab_rows), dtype=torch.float32, device=device)
    row_starts = torch.zeros(num_rows, dtype=torch.int32, device=device)
...
def sparse_attn_indexer(
    ...
    if has_decode:
        ...
        if current_platform.is_cuda() and topk_tokens & (topk_tokens - 1) == 0:
            # Every selector breaks K-th-boundary ties in launch order; repair
            # to the exact key-order set so the attended context of a decode
            # step does not depend on scheduling.
            workspace_manager = current_workspace_manager()
            (tie_workspace,) = workspace_manager.get_simultaneous(
                ((logits.shape[0], topk_tokens), torch.int32),
            )
