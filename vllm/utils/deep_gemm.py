...
def fp8_fp4_paged_mqa_logits(
    ...
    Args:
        ...
        indices: Optional request index for each varlen row. Only consumed by
            the DeepGEMM kernel; the Triton fallback (sm_8x / sm_12x) is
            dense-batch only and rejects it.
    ...
    if q_scale is None and (
        ...
        if indices is not None:
            raise NotImplementedError(
                "fp8_paged_mqa_logits_triton does not support varlen `indices`; "
                "the sm_8x / sm_12x fallback is dense-batch only."
            )
