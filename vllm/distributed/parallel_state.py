...
class GroupCoordinator:
    ...
    def _reap_completed_isends(self) -> None:
        ...
        This fork sends the metadata list synchronously ahead of the payload
        isends, so every retained handle is a device-backend tensor send whose
        ``is_completed()`` is reliable, and the list carries no gloo metadata
        handle to skip. Entries with no tensor handles at all (e.g. an empty
        ``IntermediateTensors``) are dropped immediately.
        """
        pending = getattr(self, "_pending_isends", None)
        if pending is None:
        ...
        while pending:
            ...
            if not handles:
                pending.popleft()
                continue
            if not all(h.is_completed() for h in handles):
    ...
    def isend_tensor_dict(
        ...
        # Metadata goes out on the synchronous object path, ahead of every
        # NCCL payload send, so the receiver can discover each payload shape
        # and post its receive before a send kernel occupies the device queue.
        # Sending it with isend_object instead reproduces long-lived send
        # kernels and CUDA launch-queue backpressure on deep pipeline rings.
...
def _replace_active_groups(
    *,
    ...
    function together. The caller must destroy the returned DP, EP, WORLD, and
    EPLB groups collectively and in that order. Pass all-``None`` to remove the
