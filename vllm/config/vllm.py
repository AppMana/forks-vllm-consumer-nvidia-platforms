...
DEFAULT_BREAKABLE_CUDAGRAPH_ARCHITECTURES = frozenset(
    {
        # DeepSeek V4 and the DSpark drafter are deliberately absent: their
        # attention is an opaque splitting op
        # (torch.ops.vllm.deepseek_v4_attention), so standard piecewise CUDA
        # graphs cover them. Breakable capture is lazy per batch descriptor
        # and runs gc.collect() plus empty_cache() before a per-layer
        # segmented capture; on a GB10 unified pool holding the full model
        # that stalls decode for tens of seconds per new descriptor.
        "DeepseekV32MTPModel",
...
class VllmConfig:
    ...
    def _maybe_enable_breakable_cudagraph(self) -> bool:
        ...
        # Deliberately does not force CompilationMode.NONE. Breakable CUDA
        # graphs only wrap the metadata-dependent eager attention segments;
        # the surrounding model body stays compiled. Forcing NONE here made
        # the DSV4 target and the DSpark body eager and regressed decode from
        # ~30 ms ITL to tens of seconds per output token.
    ...
    def __post_init__(self):
        ...
        # Breakable CUDA graphs wrap the eager attention boundaries while
        # torch.compile still covers the surrounding model body, so the
        # compilation mode below stays whatever the user asked for. An
        # explicit environment setting remains authoritative, keeping
        # VLLM_USE_BREAKABLE_CUDAGRAPH=0 a diagnostic opt-out.
        if self._maybe_enable_breakable_cudagraph():
            logger.info_once(
