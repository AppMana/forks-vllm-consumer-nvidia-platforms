...
def test_irecv_tensor_dict_send_allgather_postprocess_binds_keys(
    ...
    g.recv_object = lambda src: metadata_list  # type: ignore[method-assign]
...
def test_irecv_tensor_dict_can_use_preallocated_recv_buffers(
    ...
    g.recv_object = lambda src: [  # type: ignore[method-assign]
...
def test_isend_tensor_dict_sends_metadata_synchronously(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    def fake_send(t: torch.Tensor, *args: Any, **kwargs: Any) -> None:
        sent.append(t)
    ...
    # Metadata goes out on the blocking object path (size + object) ahead of
    # every payload isend, so the receiver can post its recvs first.
    assert len(sent) == 2
    assert sent[0].dtype == torch.long
    assert sent[1].dtype == torch.uint8

    # Only the payload tensor is posted asynchronously.
    assert len(posted) == 1
    assert posted[0].dtype == torch.float32

    # No metadata handle is returned; every handle is a tensor send.
    assert len(handles) == 1
...
def test_isend_tensor_dict_self_retains_for_fire_and_forget(
    ...
    assert len(handles0) == 1  # metadata is synchronous; one tensor send
    ...
    # metadata-only send (no tensors) has no handles at all and is dropped on
    # the next reap.
