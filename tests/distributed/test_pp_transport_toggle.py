...
"""PP transport toggle precedence: checkpoint stash > env var > built-in default.
...
``vllm.pp_transport`` block, which rides ``VllmConfig`` to every worker.
...
def test_pp_metadata_gate_uses_env_when_no_override(monkeypatch):
...
def test_pp_metadata_gate_override_wins_over_env(monkeypatch):
    monkeypatch.setattr(envs, "VLLM_PP_CACHE_TENSOR_DICT_METADATA", True)
    monkeypatch.setattr(kernel_config_mod, "_PP_CACHE_METADATA_OVERRIDE", False)
    assert _pp_metadata_gate(SimpleNamespace()) is False

    monkeypatch.setattr(envs, "VLLM_PP_CACHE_TENSOR_DICT_METADATA", False)
    monkeypatch.setattr(kernel_config_mod, "_PP_CACHE_METADATA_OVERRIDE", True)
    assert _pp_metadata_gate(SimpleNamespace()) is True
