...
def test_fp8_to_int8_requant_accepts_classic_fp32_block_scale():
    ...
    scale_fp32 = _e8m0_to_fp32_scale(scale_e8m0)  # same physical scale, classic dtype

    e8m0_result = requantize_fp8_to_int8_w8a16(weight_fp8, scale_e8m0)
    fp32_result = requantize_fp8_to_allspark_uint8_w8a16(weight_fp8, scale_fp32)
    # Independently exercise dequantize_fp8_block_to_bf16 with the fp32 scale
    # too (used for the wo_a passthrough path).
    bf16_direct = dequantize_fp8_block_to_bf16(weight_fp8, scale_fp32)
    ...
    assert fp32_result["qweight"].dtype is torch.uint8
