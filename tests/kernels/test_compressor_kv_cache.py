...
def test_packed_compressor_state_pages_do_not_overlap():
    ...
    config = SimpleNamespace(cache_config=SimpleNamespace(cache_dtype="int8_ds_mla"))
    specs = {
    ...
    stride = _get_kv_cache_bytes_per_block([KVCacheGroupSpec(list(specs), group_spec)])

    page = specs["compressor.0"].page_size_bytes
    assert page == specs["compressor.1"].page_size_bytes
    assert page >= specs["compressor.0"].real_page_size_bytes
    # Packed int8_ds_mla blocks are written with 16-byte vectorized stores, so
    # every layer page has to be a whole number of 16-byte units for the second
    # layer's offset to stay aligned.
    assert page % 16 == 0
