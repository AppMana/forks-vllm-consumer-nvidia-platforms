...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_indexed_d512_split_with_sink_matches_split_finish():
    torch.cuda.set_device(0)
    device = torch.device("cuda:0")
    ...
    torch.testing.assert_close(fused_max, split_max, atol=0, rtol=0)
    torch.testing.assert_close(fused_denom, split_denom, atol=0, rtol=0)
    torch.testing.assert_close(fused_output, split_output, atol=0, rtol=0)
