...
class TestDensePacking:
    ...
    def test_packed_offsets_are_16_byte_aligned_for_int8_ds_mla(self):
        ...
        config = get_kv_cache_config_from_groups(
            _mock_vllm_config("BLHNC"), groups, 1024 * 1024
        )
        by_layer = {
            name: (tensor.offset + i * tensor.layer_stride, tensor.block_stride)
            for tensor in config.kv_cache_tensors
            for i, name in enumerate(tensor.layers)
        }
        offset, block_stride = by_layer["int8_mla.0"]

        assert offset % 16 == 0
        assert block_stride % 16 == 0
