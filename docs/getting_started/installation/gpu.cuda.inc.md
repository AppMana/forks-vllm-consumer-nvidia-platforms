...
!!! note "Compiler requirement"
    Building from source requires GCC/G++ ≥ 11.3 (PyTorch's C++20 headers),
    and GCC/G++ ≥ 13 when the architecture list includes a DeepGEMM
    architecture (its `deep_jit` uses `<format>`). Ubuntu 24.04's default
    toolchain satisfies both. On Ubuntu 22.04 GCC 13 comes from the
    toolchain PPA:
    ```bash
